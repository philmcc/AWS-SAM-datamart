...
RESOURCE = 'rand_price_data_func'


def get_rand_registry_data(event, context):
    ...
    for record in rand_price_data_dict:
        print(record)
        tbl.put_item(
            Item={
        ...
        for record in rand_price_data_dict:
            ds = datetime.strptime(rand_price_data_dict[0]['date'], "%Y-%m-%d %H:%M:%S")
            ## Took a slightly different approach to the date here as the format is different from the index data feed
            ...
            key_opts = {
                ...
                'filename': str(record['address'] + '.json')
            }
            ...
            s3_url = "s3://{bucket_name}/rawpricedatadata/year={year}/month={month}/day={day}/{filename}".format(
                **key_opts)
            ...
            try:
                ...
                logger.error({
                    'resource': RESOURCE,
                    'operation': 'save price data to s3',
                    'error': str(e)
                })
                raise e
